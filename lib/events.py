...
def expire_stale_wdc_open_order_records():
    mongo_db = config.mongo_db
    min_when_created = time.mktime((datetime.datetime.utcnow() - datetime.timedelta(days=15)).timetuple())
    
    num_stale_records = config.mongo_db.wdc_open_orders.find({'when_created': {'$lt': min_when_created}}).count()
    mongo_db.wdc_open_orders.remove({'when_created': {'$lt': min_when_created}})
    if num_stale_records: logging.warn("REMOVED %i stale WDC open order objects" % num_stale_records)
    
    #call again in 1 day
    gevent.spawn_later(86400, expire_stale_wdc_open_order_records)
