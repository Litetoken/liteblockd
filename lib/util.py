...
from lib import config, util_worldcoin
...
def get_address_cols_for_entity(entity):
    if entity in ['debits', 'credits']:
        ...
        return ['issuer',]
    elif entity in ['sends', 'dividends', 'bets', 'cancels', 'callbacks', 'orders', 'burns', 'broadcasts', 'wdcpays']:
        return ['source',]
...
def decorate_message(message, for_txn_history=False):
    ...
    if message['_category'] in ['orders', 'order_matches',]:
        message['_wdc_below_dust_limit'] = (
                ('forward_asset' in message and message['forward_asset'] == config.WDC and message['forward_quantity'] <= config.ORDER_WDC_DUST_LIMIT_CUTOFF)
             or ('backward_asset' in message and message['backward_asset'] == config.WDC and message['backward_quantity'] <= config.ORDER_WDC_DUST_LIMIT_CUTOFF)
        )
    ...
    if message['_category'] in ['issuances',]:
        message['_quantity_normalized'] = util_worldcoin.normalize_quantity(message['quantity'], message['divisible'])
    return message
