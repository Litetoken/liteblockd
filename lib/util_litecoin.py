...
def get_ltc_supply(normalize=False, at_block_index=None):
    """returns the total supply of LTC (based on what litecoind says the current block height is)"""
...
def pubkey_to_address(pubkey_hex):
    ...
    address_prefix = b'\x6f' if config.TESTNET else b'\x30'
    return encoding.public_pair_to_litecoin_address(public_pair, compressed=compressed, address_prefix=address_prefix)
